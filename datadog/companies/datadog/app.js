(() => {
  ...
  const formatShortPeriod = period => {
    const match=String(period).match(/^(\d{4})Q([1-4])$/);
    return match ? `Q${match[2]}’${match[1].slice(2)}` : String(period);
  };
  ...
  const segmentationNetRead = document.getElementById("segmentation-net-read");
  const segmentationGapsList = document.getElementById("segmentation-gaps-list");
  ...
  function renderSegmentationDashboard(){
    const segmentation = kpiModel.segmentation;
    if(!segmentation || !segmentationNetRead) return;
    const latestRevenue = financials.at(-1).revenue;
    const totalGeographyRevenue = segmentation.geography.rows.reduce((sum,row)=>sum+row.current,0);
    const evidencePill = type => `<span class="evidence-pill is-${esc(type)}">${esc(type)}</span>`;
    const rangePct = (range,digits=0) => `${pct(range[0],digits).replace("%","")}–${pct(range[1],digits)}`;
    const approxCount = share => `≈${number(share*segmentation.customerSize.totalCustomers)}`;

    segmentationNetRead.innerHTML = `
      <div><span>Core product disclosures</span><b>&gt;$3.6B named ARR floors</b><p>Infrastructure, Logs, and APM + DEM are scaled; exact product mix remains undisclosed.</p></div>
      <div><span>Growth engine</span><b>North America · +${pct(segmentation.geography.rows[0].current/segmentation.geography.rows[0].prior-1,1)}</b><p>${pct(segmentation.geography.rows[0].current/totalGeographyRevenue,1)} of Q2 revenue versus ${pct(segmentation.geography.rows[1].current/totalGeographyRevenue,1)} International.</p></div>
      <div><span>Customer concentration</span><b>${pct(segmentation.customerSize.largeArrShare,0)} of ARR</b><p>Generated by only ${pct(segmentation.customerSize.largeCustomers/segmentation.customerSize.totalCustomers,1)} of customer logos.</p></div>
      <p><b>Investor conclusion:</b> non-AI revenue grew in the high-20s; existing customers generated about 70% of the Q2 revenue increase; and the &gt;$100K cohort represents about 91% of ARR. AI is a fast cross-cutting cohort, not a standalone product segment.</p>`;

    const productBody = document.querySelector("#segmentation-product [data-segmentation-body]");
    const productArrScaleMax = 2000;
    const productArrTicks = [0,500,1000,1500,2000];
    const renderProductArrRow = (row,{secondary=false}={}) => {
      const barWidth=Math.min(row.arrFloor/productArrScaleMax*100,100);
      const rowClasses=["seg-arr-row",secondary?"is-secondary":"",row.relationshipType?`is-${row.relationshipType}`:"",row.disclosureKind==="approximate"?"is-approximate":""].filter(Boolean).join(" ");
      const disclosureLabel=row.disclosureKind==="approximate"?"approximate scale":"reported lower bound";
      return `<div class="${rowClasses}" aria-label="${esc(row.name)}: ${esc(row.arrLabel)}, ${disclosureLabel}">
        <div class="seg-arr-meta"><div><span>${esc(row.name)}</span>${evidencePill(row.evidence)}</div><small>${esc(row.growthLabel)} · ${esc(row.periodLabel)}</small>${row.relationship?`<em>${esc(row.relationship)}</em>`:""}${sourceAnchor(row.sourceUrl,"Disclosure")}</div>
        <div class="seg-arr-track" style="--seg-width:${barWidth}%;--seg-color:${esc(row.color)}">
          <i class="seg-arr-fill"><b>${esc(row.chartLabel||row.arrLabel)}</b></i><span class="seg-arr-continuation" aria-hidden="true"></span>
        </div>
      </div>`;
    };
    productBody.innerHTML = `
      <div class="seg-arr-chart" aria-label="Latest management-reported product ARR disclosures on a zero to two billion dollar scale">
        <div class="seg-arr-caption"><span>Latest disclosed product ARR scale</span><b>Mixed periods · comparative scale · not product mix</b></div>
        <div class="seg-arr-axis" aria-hidden="true"><span></span><div>${productArrTicks.map(value=>`<i>${value===0?"$0":`$${(value/1000).toFixed(1)}B`}</i>`).join("")}</div></div>
        <div class="seg-arr-plot">
          <div class="seg-arr-section-label"><span>Core product families</span><b>Comparable Q4 2025 disclosures</b></div>
          ${segmentation.product.core.map(row=>renderProductArrRow(row)).join("")}
          <div class="seg-arr-section-label is-secondary"><span>Nested products &amp; growth adjacency</span><b>Different periods · do not sum with parents</b></div>
          ${segmentation.product.productSignals.map(row=>renderProductArrRow(row,{secondary:true})).join("")}
        </div>
        <div class="seg-arr-legend"><i></i><span>Solid + arrow = disclosed lower bound</span><i class="is-approximate"></i><span>Hatched = “nearing” disclosure</span><b>↳</b><span>Indented = included inside parent</span></div>
      </div>
      <div class="seg-portfolio-band">
        <span><b>5</b> products &gt;$100M ARR</span><span><b>3</b> at $50–100M</span><span><b>18</b> earlier lifecycle</span>
        ${sourceAnchor(kpiModel.productPortfolio.sourceUrl,"Q1 2026 portfolio bands")}
      </div>
      <p class="seg-footnote"><b>Disclosure boundary:</b> ${esc(segmentation.product.note)} Portfolio-band identities are undisclosed and cannot be reconciled to the named rows or the site’s broader analytical product map. ${sourceAnchor(segmentation.product.denominator.sourceUrl,segmentation.product.denominator.label)}</p>`;

    const ai = segmentation.ai;
    const scenarioNonAiRange = [1-ai.scenarioRevenueShareRange[1],1-ai.scenarioRevenueShareRange[0]];
    const aiRevenueLow = latestRevenue*ai.scenarioRevenueShareRange[0];
    const aiRevenueHigh = latestRevenue*ai.scenarioRevenueShareRange[1];
    const aiShareBars = ai.revenueShareHistory.map(row => {
      const isScenario = row.evidence === "scenario";
      const isMissing = row.aiShare == null && !row.aiShareRange;
      const aiShare = isScenario ? (row.aiShareRange[0]+row.aiShareRange[1])/2 : row.aiShare;
      const nonAiShare = aiShare == null ? null : 1-aiShare;
      const tooltip = isMissing
        ? `${formatPeriod(row.period)} — ${row.disclosureLabel}. ${row.note}`
        : `${formatPeriod(row.period)} — ${row.disclosureLabel}; non-AI ${isScenario ? rangePct([1-row.aiShareRange[1],1-row.aiShareRange[0]],1) : pct(nonAiShare,0)}. ${row.note}`;
      if(isMissing){
        return `<a class="seg-cohort-period is-missing" href="${esc(row.sourceUrl)}" target="_blank" rel="noopener noreferrer" aria-label="${esc(`${tooltip} Open source.`)}" data-tooltip="${esc(`${tooltip} Click to open source.`)}"><div class="seg-cohort-bar"><span>Not<br>stated</span></div><b>${esc(formatShortPeriod(row.period))}<i>↗</i></b></a>`;
      }
      const aiSharePct = aiShare*100;
      const rangeMarkup = isScenario ? `<i class="seg-cohort-range" style="--range-low:${row.aiShareRange[0]*100};--range-size:${(row.aiShareRange[1]-row.aiShareRange[0])*100}" aria-hidden="true"></i>` : "";
      return `<a class="seg-cohort-period ${isScenario ? "is-scenario" : "is-reported"}" href="${esc(row.sourceUrl)}" target="_blank" rel="noopener noreferrer" aria-label="${esc(`${tooltip} Open source.`)}" data-tooltip="${esc(`${tooltip} Click to open source.`)}" style="--ai-share:${aiSharePct}"><div class="seg-cohort-bar"><span class="seg-cohort-non-ai" style="height:${nonAiShare*100}%"></span><span class="seg-cohort-ai" style="height:${aiSharePct}%"></span>${rangeMarkup}<strong>${isScenario ? rangePct(row.aiShareRange,1) : pct(aiShare,0)}</strong></div><b>${esc(formatShortPeriod(row.period))}<i>↗</i></b></a>`;
    }).join("");
    document.querySelector("#segmentation-ai [data-segmentation-body]").innerHTML = `
      ...
      <div class="seg-scenario-summary"><span><b>Q2’26 analyst scenario</b><small>Not company guidance</small></span><span><b>AI ${rangePct(ai.scenarioRevenueShareRange,1)}</b><small>≈$${Math.round(aiRevenueLow)}–${Math.round(aiRevenueHigh)}M revenue</small></span><span><b>Non-AI ${rangePct(scenarioNonAiRange,1)}</b><small>${esc(ai.nonAiGrowthLabel)}</small></span></div>
      <div class="seg-metric-grid">
        <span><b>${esc(ai.incrementalRevenueContributionLabel)}</b>AI contribution to total YoY growth</span>
        <span><b>≈${rangePct(ai.incrementalRevenueShareRange)}</b>of incremental revenue under a 7–9 ppt convention</span>
        <span><b>≈${number(ai.customers)}</b>AI-native customers</span>
        <span><b>${number(ai.customersAbove1mAnnualSpend)} / ${number(ai.customersAbove10mAnnualSpend)}</b>&gt;$1M / &gt;$10M annual spend</span>
      </div>
      <div class="seg-exposure-callout"><b>≈${number(ai.integrationCustomers)} customers · ≈${pct(ai.integrationArrShare,0)} of ARR</b><p>Accounts sending data from an AI integration. This measures workload exposure, not AI revenue; roughly ${pct(ai.integrationCustomerShare,0)} of logos represent ${pct(ai.integrationArrShare,0)} of ARR.</p>${sourceAnchor(ai.integrationSourceUrl,"Q1 2026 exposure")}</div>
      <p class="seg-footnote">${evidencePill(ai.evidence)} ${esc(ai.note)} ${sourceAnchor(ai.priorSourceUrl,"Historical mix")} ${sourceAnchor(ai.currentSourceUrl,"Q2 2026 bridge")}</p>
      <p class="seg-cohort-definition"><b>Who counts as AI-native?</b> ${esc(ai.cohortDefinition)} This is not the broader group of customers that merely sends data from an AI integration. ${sourceAnchor(ai.cohortDefinitionSourceUrl,"Management description")}</p>`;

    document.querySelector("#segmentation-geography [data-segmentation-body]").innerHTML = `
      <div class="seg-geo-list">${segmentation.geography.rows.map(row=>{
        const share=row.current/totalGeographyRevenue;
        const growth=row.current/row.prior-1;
        return `<div class="seg-geo-row"><div><span>${esc(row.name)}</span><b>${money(row.current)}</b></div><div class="seg-bar"><i style="width:${share*100}%;--seg-color:${esc(row.color)}"></i></div><div><strong>${pct(share,1)} mix</strong><i>+${pct(growth,1)} YoY</i></div></div>`;
      }).join("")}</div>
      <div class="seg-divergence"><span>Growth gap</span><b>${((segmentation.geography.rows[0].current/segmentation.geography.rows[0].prior-segmentation.geography.rows[1].current/segmentation.geography.rows[1].prior)*100).toFixed(1)} ppt</b><p>North America is currently expanding materially faster than International.</p></div>
      <p class="seg-footnote">United States alone: ${money(segmentation.geography.unitedStates.current)}, ${pct(segmentation.geography.unitedStates.share,1)} mix, +${pct(segmentation.geography.unitedStates.growth,1)} YoY. Basis: ${esc(segmentation.geography.basis)}. ${sourceAnchor(segmentation.geography.sourceUrl,"Q2 2026 Form 10-Q")}</p>`;

    const customer = segmentation.customerSize;
    const largeLogoShare = customer.largeCustomers/customer.totalCustomers;
    const subCustomers = customer.totalCustomers-customer.largeCustomers;
    const subPrior = customer.priorTotalCustomers-customer.priorLargeCustomers;
    document.querySelector("#segmentation-customer-size [data-segmentation-body]").innerHTML = `
      <div class="seg-customer-matrix">
        <div class="seg-matrix-head"><span>Cohort</span><span>Logo mix</span><span>ARR mix</span><span>Logo growth</span></div>
        <div><span>&gt;$100K ARR<br><small>≈${number(customer.largeCustomers)} customers</small></span><b>${pct(largeLogoShare,1)}</b><strong>${pct(customer.largeArrShare,0)}</strong><i>+${pct(customer.largeCustomers/customer.priorLargeCustomers-1,1)}</i></div>
        <div><span>&lt;$100K ARR<br><small>≈${number(subCustomers)} customers</small></span><b>${pct(1-largeLogoShare,1)}</b><strong>${pct(1-customer.largeArrShare,0)}</strong><i>+${pct(subCustomers/subPrior-1,1)}</i></div>
      </div>
      <div class="seg-metric-grid">
        <span><b>${number(customer.millionCustomers)}</b>&gt;$1M ARR customers · ${customer.millionCustomerPeriod}</span>
        <span><b>+${pct(customer.millionCustomers/customer.priorMillionCustomers-1,0)}</b>YoY growth in &gt;$1M cohort</span>
        <span><b>${pct(customer.fortune500Penetration,0)}</b>Fortune 500 penetration</span>
        <span><b>&lt;$${customer.fortune500MedianArrUpperBound.toFixed(1)}M</b>median Fortune 500 ARR</span>
      </div>
      <p class="seg-footnote">Counts and mix are approximate disclosures; sub-$100K values are derived residuals. ${sourceAnchor(customer.sourceUrl,"Q2 2026 filing")} ${sourceAnchor(customer.millionSourceUrl,"FY2025 cohort")} ${sourceAnchor(customer.fortune500SourceUrl,"Fortune 500 whitespace")}</p>`;

    const bridge = segmentation.growthBridge;
    const bridgeRows = [bridge.q2,bridge.firstHalf];
    const bridgeScaleMax = Math.ceil(Math.max(...bridgeRows.map(row=>row.revenueIncrease))/100)*100;
    const bridgeTicks = [bridgeScaleMax,bridgeScaleMax/2,0];
    const bridgePeriodLabel = period => period === "2026Q2" ? "Q2’26" : period.replace(" 2026","’26");
    const bridgeBars = bridgeRows.map(row => {
      const existingDollars=row.revenueIncrease*row.existingShare;
      const newDollars=row.revenueIncrease*row.newShare;
      const totalHeight=row.revenueIncrease/bridgeScaleMax*100;
      const tooltip=`${bridgePeriodLabel(row.period)} — ${money(row.revenueIncrease)} total year-over-year revenue increase. Existing customers contributed approximately ${pct(row.existingShare,0)} (${money(existingDollars)} derived); new customers contributed approximately ${pct(row.newShare,0)} (${money(newDollars)} derived).`;
      return `<div class="seg-growth-period" tabindex="0" role="img" aria-label="${esc(tooltip)}" data-tooltip="${esc(tooltip)}" style="--growth-total:${totalHeight}"><div class="seg-growth-bar"><span class="seg-growth-new" style="height:${newDollars/bridgeScaleMax*100}%"><b>≈$${Math.round(newDollars)}M</b></span><span class="seg-growth-existing" style="height:${existingDollars/bridgeScaleMax*100}%"><b>≈$${Math.round(existingDollars)}M</b></span><strong>${money(row.revenueIncrease)}</strong></div><b>${esc(bridgePeriodLabel(row.period))}</b><small>${pct(row.existingShare,0)} existing · ${pct(row.newShare,0)} new</small></div>`;
    }).join("");
    document.querySelector("#segmentation-new-existing [data-segmentation-body]").innerHTML = `
      <div class="seg-growth-chart" aria-label="Year-over-year revenue increase attributed to existing and new customers">
        <div class="seg-growth-chart-head"><div><b>YoY revenue increase by source</b><span>Reported total increase · attributed mix converted to derived dollars</span></div><button class="info-tip" type="button" aria-label="Growth source chart methodology" data-tooltip="Q2 2026 and H1 2026 are overlapping measurement windows, not sequential periods. Total revenue increases are exact differences from filed revenue. Existing/new percentages are approximate company attributions, so the segment dollars are derived approximations.">i</button></div>
        <div class="seg-growth-legend"><span><i class="is-existing"></i>Existing customers</span><span><i class="is-new"></i>New customers</span></div>
        <div class="seg-growth-plot">${bridgeTicks.map((tick,index)=>`<div class="seg-growth-gridline is-${index}"><span>$${Math.round(tick)}M</span></div>`).join("")}<div class="seg-growth-bars">${bridgeBars}</div></div>
      </div>
      <p class="seg-footnote">${esc(bridge.note)} Q2 and H1 overlap; compare mix and cumulative scale, not sequential momentum. Percentages are approximate company attributions; segment dollars are derived. ${sourceAnchor(bridge.sourceUrl,"Q2 2026 Form 10-Q")}</p>`;

    const multi = segmentation.multiProduct;
    document.querySelector("#segmentation-multi-product [data-segmentation-body]").innerHTML = `
      <div class="seg-adoption-list">${multi.rows.map(row=>`<div class="seg-adoption-row"><span>${esc(row.label)}</span><div class="seg-adoption-track"><i style="width:${row.current*100}%"></i><em style="left:${row.prior*100}%" title="${esc(multi.priorPeriod)}: ${pct(row.prior,0)}"></em></div><b>${pct(row.current,0)}</b><strong>+${((row.current-row.prior)*100).toFixed(0)} ppt YoY</strong><small>${approxCount(row.current)} customers</small></div>`).join("")}</div>
      <div class="seg-adoption-legend"><span><i></i>${esc(multi.currentPeriod)}</span><span><em></em>${esc(multi.priorPeriod)} marker</span><p>${esc(multi.note)}</p>${sourceAnchor(multi.sourceUrl,"Q2 2026 filing")}</div>`;

    segmentationGapsList.innerHTML = segmentation.disclosureGaps.map(gap=>`<span>${esc(gap)}</span>`).join("");
  }
  ...
  function revealDeepLink(targetId,{behavior="auto"}={}){
    if(TAB_IDS.includes(targetId)){
      ...
      document.querySelectorAll(".is-deep-linked").forEach(item => item.classList.remove("is-deep-linked"));
    ...
    requestAnimationFrame(() => requestAnimationFrame(() => {
      const card = target.closest(".kpi-chart-card, .seg-cohort-chart, .segmentation-card") || target;
      document.querySelectorAll(".is-deep-linked").forEach(item => item.classList.remove("is-deep-linked"));
